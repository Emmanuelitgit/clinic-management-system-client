...
export default function ManageAppointment({name,id,patient_id,doctor_id,patient_name,date,doctor_name,desc,title}) {
  ...
  const handleUpdate = async() => {
    try {
      const response = await api.put(`/update_appointment/${id}`, data);
  ...
  const handleDelete = async () => {
    ...
    swalWithBootstrapButtons.fire({
      ...
      if (result.isConfirmed) {
        try {
          const response = await api.delete(`/remove_appointment/${id}`);
          if (response.status === 200) {
            handleDepCount();
            swalWithBootstrapButtons.fire("Deleted!", "Appointmnet deleted successfully.", "success");
          }
        } catch (error) {
          swalWithBootstrapButtons.fire("Error!", "Appointment could not be deleted.", "error");
        }
